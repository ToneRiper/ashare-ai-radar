...
def get_5min_spikes():
    """V66核心：抓取全市场最新5分钟内火箭发射/垂直拉升的个股，判断资金真实攻击方向"""
    try:
        ...
        # f11是5分钟涨跌幅，筛选5分钟内拉升超 1.5% 的异动股
        spikes = [f"{s['f14']}(5分钟拉升:{s['f11']}%, 目前涨:{s['f3']}%)" for s in data if s.get('f11') and s['f11'] > 1.5]
        return " | ".join(spikes) if spikes else "当前盘面无明显5分钟激增异动"
    except:
        return "5分钟异动数据监控中..."
...
def generate_dashboard(topic_counts, review_text, today_str):
    ...
    html_content = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>游资暗潜雷达</title><script src="https://cdn.jsdelivr.net/npm/echarts@5.4.3/dist/echarts.min.js"></script><style>body{{background-color:#0f172a;color:#e2e8f0;font-family:sans-serif;padding:20px;}} .container{{display:flex;flex-wrap:wrap;gap:20px;}} .card{{background:#1e293b;border-radius:12px;padding:20px;flex:1;min-width:300px;}} pre{{white-space:pre-wrap;color:#cbd5e1;line-height:1.6;}}</style></head><body><h2>📊 A股联动全透大屏 (V66)</h2><p>更新时间：{today_str}</p><div class="container"><div class="card"><h2>🔥 产业链热力图</h2><div id="chart" style="width:100%;height:400px;"></div></div><div class="card" style="flex:2;"><pre>{review_text if review_text else "暂无推演"}</pre></div></div><script>var chart=echarts.init(document.getElementById('chart'));chart.setOption({{tooltip:{{}},series:[{{type:'pie',radius:['40%','70%'],data:{pie_data_str}}}]}});</script></body></html>"""
    try:
        with open("index.html", "w", encoding="utf-8") as f: f.write(html_content)
    except: pass

# ======================
# 4. 游资大脑核心 AI 引擎 (植入盘面共振联动逻辑)
# ======================
def get_semantic_intraday_alert(news_list, top_sectors, spikes_5min, focus_keywords, mode):
    news_text = "\n".join(news_list[:20])
    prompt = f"""你是A股游资。模式：【{mode}】。
资金大风向：{top_sectors}
【极短期微观异动】：最近5分钟全市场爆拉个股：{spikes_5min}
自选核心库：{focus_keywords}

【联动验证铁律】：
1. 将【新闻线索】与【5分钟微观异动】进行交叉比对！如果新闻出的利好，刚好在5分钟异动名单里有体现，说明“资金已点火共振”；如果没体现，说明资金不认可或尚未发酵。
2. 深度拷问：为什么这时候拉升？是掩护出货还是真突破点火？
3. 必须推荐 4-5 只 50亿-300亿 极度活跃、有连板基因的小盘先锋股，严禁大盘股。

快讯：{news_text}

严格输出：
【微观联动研判】(结合新闻与5分钟异动，一针见血指出资金真实攻击线)
【刀尖深度拷问】(揭露主力意图或圈套)
【狙击尖刀标的】(格式：代码 名字，给足4-5只)
【盘中防雷警示】(指出核按钮退潮风险)"""
    try:
...
按【N字反包战法】（1.近一周有涨停; 2.近两日缩量回调且绝对不破涨停底; 3.无跌停; 4.今日下影线承接且换手活跃; 5.市值50-300亿）挖5只潜力妖股。只输出5个6位代码，逗号隔开。"""
    try:
...
def get_daily_review(news_list, top_sectors):
    news_text = "\n".join(news_list[:35])
    prompt = f"""晚上大复盘。线索：{news_text}。资金：{top_sectors}。
格式：
【宏观大局观】情绪周期。
    ...
    if hour == 14 and 45 <= minute <= 59:
        ...
        if ambush_list:
            for data in ambush_list[:5]:
                msg += f" • {data['name']}({data['code']}) | 跌跌:{data['change']}% | 换手:{data['turnover']}% | 量比:{data['vol_ratio']}\n"
